...
router.post('/profile', async (req, res) => {
    ...
    if (user_result.length == 0) {
        ...
        return_user_result.status = (return_user_result.paid_valid_until > new Date())? "Hạng cao cấp" : "Hạng miễn phí"
        res.send({
            status: 'success',
...
router.post('/billing', async (req, res) => {
    ...
    let token_result = db.queryRecord("session", token_query)
    if (token_result.length == 0) {
    ...
    billing_list_result.forEach((val) => {
        let billing_entry = {
